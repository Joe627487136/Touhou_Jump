...
public class Sum_manager : MonoBehaviour {
	...
	void Start () {
		...
		total_char_count = Loader_cha_sheet.Count;
		register_index_list_rework ();
		Get_imgspritelist ();
	}
	...
	void Get_imgspritelist(){
		...
		int children_count = total_char_count;
	...
	void register_index_list_rework(){
		...
		for (int i=0; i<total_char_count; i++){
			...
			else if (Current_char_rarity == "R") {
				r_index.Add (i);
			}
			else if (Current_char_rarity == "SR") {
				sr_index.Add (i);
			}
		}
